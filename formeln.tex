...
\newcommand{\mysection}[1]{\hr\vspace{-0.15cm}\section{\fcolorbox{white}{green}{#1}}}
...
\newcommand{\mydoubleeqarrow}[2]{\begin{align*}\boxed{#1} & \hfill & {\Large \myto} & \hfill & \boxed{#2}\end{align*}}
...
$a = \lim_{n\to\infty} \sqrt{a+2} \myto a = \sqrt{a+2}$
...
\bmyeq{f(x) = \ln(x + a) \myto f'(x) = \frac{1}{x + a}}
...
\bmyeq{\partialvar{x}{\frac{\sqrt{x+a}}{x+b}} = \frac{-2a+b-x}{2\sqrt{a+x}(b+x}^2}

\mysection{Integration}

\bmyeq{\int x^n \dx = \frac{1}{n+1}x^{n+1} + C}

\bmyeq{\int c\cdot f(x)\dx = c \cdot \int f(x) \dx}

\bmyeq{\int\left(f(x) +g(x)\right) \dx = \int f(x)\dx + \int g(x)\dx}

\bmyeq{\int f'(x)g(x)\dx = f(x)\cdot g(x) - \int f(x)\cdot g'(x)\dx}

\bmyeq{\int \frac{f'(x)}{f(x)} \dx = \ln(f(x)) + C}

\bmyeq{\int \frac{1}{u^2} \du = \frac{1}{u}}
...
\mydoubleeq{\sum_{n = 0}^\infty \frac{x^n}{n!} = e^x}{\sum_{n = 0}^\infty \frac{g}{a^n} = g\cdot \frac{a}{a-1} \text{ (für } |a| > 1\text{)}}
...
\bmyeq{\sum_{n=0}^\infty \left(-1\right)^n\cdot z^n = \frac{1}{1+z} \text{ (für }|z| < 1\text{)}}

\bmyeq{\sum_{n=0}^\infty \frac{a}{b^n} = \frac{ab}{b-1} \text{ (für }|b|>1\text{)}}

\bmyeq{\sum_{n=0}^\infty \frac{a^{n+1}}{b^n} = \frac{ab}{b-a} \text{ (für }|a|<|b|\text{)}}

\bmyeq{\sum_{n=0}^\infty \frac{a^{n-1}}{b^n} = \frac{b}{a(b-a)} \text{ (für }|a|<|b|\text{)}}

\bmyeq{\sum_{n=0}^\infty \frac{a((-b)^n)}{c^n} = \frac{ca}{b+c} \text{ (für }|b|<|c|\text{)}}

\bmyeq{\sum_{n=1}^\infty\frac{(-1)^{n+1}}{n} = \ln(2) \text{ (Alt. harm. R.)}}

\bmyeq{\sum_{n = 0}^\infty \frac{(-1)^n}{2n+1} = \frac{\pi}{4} \text{ (Leibnitzreihe)}}

\bmyeq{\sum_{n=0}^\infty x^n = \frac{1}{1-x} \text{ (Geometrische Reihe)}}

\bmyeq{\sum_{n=0}^\infty\frac{1}{n} = \infty \text{ (Harmonische Reihe)}}

\bmyeq{\sum_{n = 0}^\infty (-1)^n \frac{x^{n+1}}{n+1} = \ln(1 + x) \text{(für }x \in (-1, 1]\text{)}}


\[
\boxed{
	\displaystyle
	\sum_{n = 0}^\infty \frac{a^n}{b^n} = \sum_{n = 0}^\infty \left(\frac{a}{b}\right)^n = \begin{cases} %
...
\mysection{Höhenlinien}

$z = f(x, y)$, Höhenlinien $c = 0, 2, 4, \dots$ $\to$ $0 = f(x, y)$ usw.
...
\mysection{Wichtige Werte}

\mydoubleeq{\sin(0) = 0}{\cos(0) = 1}

\mydoubleeq{\sin\left(\frac{\pi}{2}\right) = 1}{\cos\left(\frac{\pi}{2}\right) = 0}
\mydoubleeq{\sin\left(\frac{-\pi}{2}\right) = -1}{\cos\left(-\pi\right) = -1}
...
\mysubsection{Induktion Folge}

\mydoubleeq{a_{n+1} = 1-\sqrt{1-a_n}}{a_0 = \frac{3}{4}}

$a_{n+1}$ ist beschränkt durch 0 und 1. 

\noindent Zu zeigen: $ 0 < 1 - \sqrt{a_{n+1}} < 1 $


\bmyeq{\text{IA: } 0 < a_n < 1}

\mydoubleeqarrow{\text{IS: } 0 < a_n < 1}{| \cdot (-1) | \cdot +1 | \sqrt{\phantom{\square}} | \cdot (-1) | + 1}

\mydoubleeqarrow{0 < 1 - \sqrt{1-a_n} < 1}{0 < 1 - a_{n+1} < 1 \blacksquare}
